...
class CANStats:
    ...
    def update(self, can_id):
        now = time.time() * 1000  # milliseconds
        entry = self.stats.get(can_id)
        if entry is None:
            self.stats[can_id] = {"count": 1, "last_time": now, "total_cycle": 0}
        else:
            cycle = now - entry["last_time"]
            entry["last_time"] = now
            entry["count"] += 1
            entry["total_cycle"] += cycle

    def table(self):
        lines = [f"{'CAN_ID':>10} {'Cycle Time (ms)':>15} {'Count':>6}"]
        for can_id, entry in self.stats.items():
            count = entry["count"]
            avg_cycle = entry["total_cycle"] / (count - 1) if count > 1 else 0
            lines.append(f"{hex(can_id):>10} {avg_cycle:>15.2f} {count:>6}")
        return "\n".join(lines)
