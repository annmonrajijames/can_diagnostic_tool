    ...
    def load_dbc(self):
        ...
        if file_path:
            try:
                ...
                # Strip malformed message blocks (missing name etc.)
                ...
                msg_count = len(self.db.messages)
                sig_count = sum(len(m.signals) for m in self.db.messages)
                self.status_label.setText(
                    f"Loaded: {file_path} ({msg_count} msgs, {sig_count} signals)"
                )
